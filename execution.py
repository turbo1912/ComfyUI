...
def recursive_execute(
    server, prompt, outputs, current_item, extra_data, executed, prompt_id, outputs_ui
):
    ...
    def execute(self, prompt, prompt_id, extra_data={}, execute_outputs=[]):
        ...
        if self.server.client_id is not None:
            self.server.send_sync(
                "execution_start", {"prompt_id": prompt_id}, self.server.client_id
            )
        ...
        with torch.inference_mode():
            ...
            while len(to_execute) > 0:
                ...
                # error was raised
                success, error, ex = recursive_execute(
                    self.server,
                    prompt,
                    self.outputs,
                    output_node_id,
